...
const fetchCoins = (state, action) => {
    return updateObject(state, {coins: action.coins, selectedCrypt: action.coins[0]});
}
const setCurrency = (state, action) => {
    ...
    updatedCoins = updatedCoins.map((coin) => {
      let copiedCoin = {...coin};
      coin.name === action.currencyName ? (copiedCoin.checked = true) && (updatedSelectedCrypt = {...copiedCoin}) : copiedCoin.checked = false;
      return copiedCoin;
    })
