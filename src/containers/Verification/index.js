import React, { Component, Figure } from 'react';
...
class Verification extends Component{
    ...
    componentWillMount(){
      let navBtns = [], necessaryKeys = 'User details,Address,Bank';
      navBtns = propertiesExtractor(this.state.userData, necessaryKeys, [], true);
      let updNavBtns = navBtns.map((nav, index) => {
        nav.checked && this.setState({selectedSection: nav});
        let borderTextColor = nav.status === 'Verified' ? '#6bcc00' : '#c6c6c6';
        let content = <div className={classes.NavBtn}>
                        <div style={{color: borderTextColor, borderColor: borderTextColor}}>{index+1}</div>
                        <div>
                          <div>{nav.title}</div>
                            {nav.status === 'Verified' ?
                            ...
                            : null}
                            {nav.status === 'Is being verified' ? <div style={{color:'#627eea'}}>{nav.status}</div> : null}
                            {nav.status === 'Not verified' ? <div style={{color:'#e76071'}}>{nav.status}</div> : null}
                        </div>
                      </div>
        nav.content = content;
        return nav;
